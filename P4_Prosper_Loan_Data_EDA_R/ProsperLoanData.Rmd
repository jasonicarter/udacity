...
# TODO Look at the top 10 states and show factors from LoanStatus
```
...
grp_by_state <- prosperloans %>%
  ...
  summarise(count = n()) %>%
  filter(DelinquentBorrowers == True)
...
ggcorr(prosperloans, label = TRUE, label_size = 3, hjust = 0.75, size = 3, color = "grey50")

names(prosperloans)
ggplot(prosperloans, aes(CreditScoreRangeLower, BorrowerRate)) +
  geom_point(alpha = 0.1)


# OR just use built in cor and corrplot
#library(corrplot)
#M <- cor(mtcars)
