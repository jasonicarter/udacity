...
```{r}
# ListingCreationDate
# ListingCreationDate fill=loan status

#strip out year from date 
ggplot(head(prosperloans), aes(ListingCreationDate))


# d <- as.Date('3/2/2015 19:09:29.263000','%m/%d/%Y')
# format(d,"%Y")

d <- head(prosperloans)
qplot(ListingCreationDate, data=d)
```


Time to further the analysis and some other interesting initial features to look at in regards to delinquency are: Term, LoanStatus, LoanOriginalAmount, Income Range and Borrower's State.
...
Continuing to look for trends in delinquency, I will investigate possible expected and unexpected relationships between features.
