...
Some interesting summary variables to look at: Term, LoanOriginalAmount and Investors
...
The average term appear to be **36 months (3 yrs)** with an original loan request being **$8337**, ranging from $1000 to 35000. This seems to cover the full available range allowed by Prosper, although at the time of writting this analysis, Prosper has a minimum limit of $2000. And what seems like a surprising amount, on averager these loans have **80 investors**.

```{r echo=FALSE}
# histogram plot of 
# Term (count)
# LoanOriginalAmount(cound) - see if any trends in loan requests
# IncomeRange???
# BorrowerState 
# filter out those who are >120, defaulted or chargedoff
# layer them on top of each other good - green, deliquent - red
# still univariate, against count, using layers
test <- factor(prosperloans$LoanStatus,levels = c("Defaulted","Chargedoff","Past Due (91-120 days)","Past Due (>120 days)"))
ggplot(prosperloans,aes(Term, fill=LoanStatus)) + 
  geom_histogram(binwidth = 12) +
  scale_x_continuous(breaks = c(0,12,36,60))

ggplot(prosperloans,aes(IncomeRange)) + geom_bar() # need to re-order and maybe drop off values
ggplot(prosperloans,aes(BorrowerState)) + geom_bar() # remove states with 0? 
