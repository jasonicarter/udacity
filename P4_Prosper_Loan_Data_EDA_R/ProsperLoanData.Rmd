...
Here I dive into the examining the relationship between multiple variables of the dataset. There are some expected "strong" relationships which will be investigated but also any unusual or unexpected relationships strong or weak relationships will also be called out for investigation as well.

First, to start with, a correlation matrix will be calculated and used in order to help start the investigation where by the variables with strong and/or weak relationships will be reviewed.
...
g1 <- ggplot(subset(prosperloans, DelinquentBorrowers == 1), 
             aes(CreditScoreRangeLower, LoanOriginalAmount)) +
...
Discuss - expected higher credit score, the higher the loan amount...partially due, but still the majority of borrowers even with higher credit score borrow under $20k
